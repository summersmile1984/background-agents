...
resource "terraform_data" "cloudflare_control_plane_custom_domain_gate" {
  lifecycle {
    precondition {
      condition     = local.control_plane_custom_domain == "" || local.control_plane_custom_domain_enabled
      error_message = "cloudflare_control_plane_custom_domain is set but would be silently ignored: it requires a non-empty cloudflare_zone_id."
    }
  }
}
