...
import type { AgentRuntimePreferencesStore } from "../db/agent-runtime-preferences";
...
export async function resolveAgentHarness(options: {
  ...
  runtimePreferencesStore?: AgentRuntimePreferencesStore;
  ...
  const runtimePreferences = await options.runtimePreferencesStore?.get();
  if (runtimePreferences) return runtimePreferences.defaultAgentHarness;
