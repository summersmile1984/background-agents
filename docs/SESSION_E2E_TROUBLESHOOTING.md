...
| A native harness reports `HTTP 409: Approval policy cannot be configured at session scope` when spawning a child | The harness attempted to override approval policy for a child session, which the control plane does not allow    | Treat child spawning as an optional capability; continue the parent task or use an explicitly configured deployment-level policy, and do not retry recursively          | The parent session completes without unbounded child sandboxes or duplicate work                                                    |
