...
- 事件入口仍先返回 Feishu 的 HTTP ACK，避免长任务阻塞平台重试；随后发送的“已收到”回执现在使用
  `feishu:<message-id>:receipt` 幂等键。命令回执和命令结果也分别使用消息级键。
...
- Feishu bot 回归现为 123 项通过；全局 TypeScript
  typecheck 和 lint 通过。生产发布及真实飞书消息验证将在本提交的 CI/Terraform 完成后记录。
