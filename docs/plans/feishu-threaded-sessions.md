...
- [x] 窄屏 Web 保留当前按钮分页，不恢复会被输入法遮挡的下拉选择器。
- [x] 仓库选择后按 readiness
...
- [x] 跨用户和未过期旧卡片负向路径保留服务端校验；按当前范围不要求第二个飞书身份的生产验收（未绑定、未 @ 群消息忽略已在 7.1 验证）；
- [x] 完成飞书 Web 验收（桌面浏览器与窄屏响应式视口）；原生手机 App 不在本次范围；
...
- 本轮证明“双 SCM 两话题隔离”“同一话题续办不重选仓库、不换 session”和旧卡无法控制 session。截图/preview/PR 归属、跨用户、未过期旧卡和私聊双 session 仍按下文 Runbook 验收；原生手机 App 不在本次范围；rollout 回滚演练已在 7.15 完成。
...
- 本轮只验证消息回执、Gitea 目录分页、仓库绑定、Harness/模型/Effort 选择和完成消息；截图、preview、PR 及跨用户仍按后续验收项跟踪，原生手机 App 不计入本次发布范围。
...
URL 和桌面/窄屏 Web 截图；日志中不得保存 prompt 正文或 token。
...
1. （可选）第二个测试用户在 A 话题续办，确认被拒绝且不投递 prompt；
...
4. 在窄屏 Web 视口完成代码源、仓库分页选择和两个话题切换；
5. 确认窄屏布局中的按钮可见，工作卡能辨认 repo/branch/session。原生手机 App 不在本次验收范围。
...
- 对应 session `7777312f060800528e36e32fceedc1ac` 最终完成；运行时确认分支 commit
  ...
  `1440×900`（viewport）与窄屏 Web 场景 `390×844`（full-page）的断言全部通过。
- 两张截图均由沙盒标准 visual-verification 流程上传并回到原 Feishu 话题：artifact
  `374b5d825360828969c44d9ae8e29929`（桌面）和
  `0e8dedf674227864e151911b08d60d95`（窄屏 Web）。完成卡显示“视觉验证已通过：2 个场景，2 张截图”，并提供“打开预览”入口。
- 预览网关返回 HTTP 200；可访问地址为
  `https://preview.89347589.org/sandbox/9f1c820121a647509c4842cdfd9c895e/4173/`，窄屏 Web 场景为同路径下的
...
- 本次没有重新出现代码源/仓库选择卡，也没有创建新会话；因此确认当前生产版本的命令回执、稳定 UUID 和已绑定 Gitea 话题路由仍然正常。记录该验证的提交
  `0c8c60b6` 已通过 CI run
  `33215313654`。该冒烟不替代当时完成定义中尚未完成的跨用户验收；原生手机 App 已按 2026-08-30 的范围调整移出本次发布条件。
...
- 本次复核未改变仓库、会话或连接配置，仅补充运行证据；跨用户未过期旧卡片仍保持未完成，原生手机 App 真机验收按当前范围不作为发布条件。
...
- 该回归证明最新原生 Harness 超时修复已部署且不影响已绑定 Gitea 话题的续办路径；跨用户/未过期旧卡片仍按完成定义保持未完成，原生飞书手机 App 不属于本次发布验收范围。

### 7.19 Gitea 真实视觉回归与声明门禁（2026-08-29）

- 在同一已绑定的 Gitea `huangdong/chatbi`
  话题再次发起只读视觉请求。飞书先收到工作回执，随后在约 4 分钟内收到完成卡；原生 Codex
  stream 没有再次出现无限 `processing`。Harness 实际启动了 `web` 服务并生成桌面 `1440×900`
  和窄屏 Web `390×844` 两张截图，工作区保持干净。
...
- 提交 `00305f30` 已推送到 `main`；Terraform 生产发布 `33303080810` 已成功，Control Plane、Feishu
  ...
  127 项测试、typecheck、Prettier 和 ESLint 均通过。窄屏 Web 验收已完成；原生飞书手机 App 后续如需支持再安排真机测试。
...
- 按当前范围，跨用户/未过期卡片的真实拒绝路径仍需要第二个飞书身份；Web 桌面与窄屏响应式证据已足够，原生手机 App 不属于发布验收条件。
...
- 已知不可重试的模板/配置类 `SandboxProviderError` 现在会立即失败当前 pending prompt，并广播明确的
  `prompt.dispatch_failed`；配额、鉴权或网络类错误仍按 15 分钟 dispatch
  deadline 等待重连。这样不会把可恢复的 provider 故障误判为终态，同时配置错误也不再让用户等待超时。
...
- [x] 未绑定群消息不触发；
- [x] 跨用户和未过期旧卡片保留 actor/action/pending 校验；第二个飞书身份的生产负向验收按当前范围不要求；
- [x] 私聊两个 session 不依赖隐式当前会话且可以显式续办（可用 `#短编号 请求` 指定）；
- [x] 飞书 Web 桌面与窄屏响应式视口截图验证通过；
- [x] 飞书 Web 桌面与窄屏响应式视口的按钮、卡片和截图验证通过；原生手机 App 明确标记为本次范围外；
