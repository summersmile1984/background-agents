...
— 生产线程路由、单话题续办、单仓库绑定、视觉截图/预览回传、私聊多会话和 rollout 回滚 E2E 已验收；当前只剩跨用户负向路径待完成。飞书原生移动 App 不在本次发布验收范围内。**

> 验收范围（2026-08-30 更新）：本版本以飞书 Web（桌面浏览器及窄屏响应式视口）为支持目标；原生飞书手机 App 的键盘遮挡和按钮操作不作为发布阻塞项，后续如需支持再单独安排真机验收。
