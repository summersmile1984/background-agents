...
- **Browser automation:** agent-browser plus an AIO Sandbox Chromium/CDP/Browser MCP runtime for
  screenshots, visual diffs, interactive browser tools, and UI verification. On Cube, the browser
  services stay loopback-only inside the per-session VM; screenshots return through Open-Inspect's
  authenticated media pipeline
